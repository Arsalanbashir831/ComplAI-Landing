...
export default function NewsSection() {
    ...
    if (loading) {
        return <div className="container mx-auto px-4 py-8">Loading news articles...</div>
    }
                    ...
                    <div className="flex flex-col  md:flex-row gap-8 items-center">
                        ...
                        <div className="flex flex-col items-start py-4 justify-between w-full md:w-1/2 space-y-4  h-96">
                            <div className=" gap-2 text-sm text-gray-400 py-1.5 rounded-full bg-[#E2ECFF] w-36">
                                <span className="bg-blue-600 text-white  px-2 py-1 m-1 rounded-full ">Latest</span>
                                    {calculateReadTime(featuredBlog.content)} min read
                               
                            </div>
                            <h2 className="text-3xl md:text-4xl font-normal text-gray-900">{featuredBlog.title}</h2>
                            <p className="text-gray-700">{parseBodyContentToText(featuredBlog.content, 250)}</p>
                            <Link
